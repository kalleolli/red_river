...
load(file = paste0(reddir, '/dat/bmr_all.rda')) # loads  bmr_all; created in red_river_4.R

# shrink down all_imp_mcl to save space; ale stands for Accumulated Local Effects
ale.imp <- lapply(all_imp_mcl, '[[', 2) # 52 Mb
rm(all_imp_mcl)

# bmr_tuned_ag0 <- bmr_tuned_ag
bmr_tuned_ag <- filter(bmr_all, hyp_id != '0') # tuned models only; 1494000 x 9

ale.df <- lapply(seq_along(ale.imp), function(i){bind_cols(ale.imp[[i]], bmr_tuned_ag[i, ])} ) %>% 
  bind_rows() %>% 
  filter(batch_id != '0') %>% 
  mutate(hyp_id = factor(hyp_id, levels = c('D','1','2'), labels = c('Default','rbv1','rbv2') )) # 1494000 x 13

# select
idx <- which(bmr_tuned_ag$feat_id == 'fk' & bmr_tuned_ag$red_id == 'Red' & bmr_tuned_ag$mod_id == 'rg' & bmr_tuned_ag$batch_id != '0') # length 150 = 3 mod_id x 50 hyp-headers
...
for(i in 1:length(feats)){
  ...
  ale_fig_lst[[i]] <- ggplot(filter(ale_fig_base, feature == feats[i]), aes(x = X, y = value, col = hyp_id)) +
    geom_line(aes(group = paste0(nr, hyp_id)), alpha = .05) +
    ...
    labs(title = featnames[i], x = 'Variable value', y = 'Relative importance')
}
...
for(i in 1:length(feats)){
  ...
  ale_figS_lst[[i]] <- ggplot(filter(ale_fig_base, feature == feats[i]), aes(x = X, y = value, col = hyp_id )) + 
    geom_line(aes(group = paste0(nr, hyp_id)), alpha = .05) +
    theme(legend.position="none") +
    geom_rug(mapping = aes(x = X, y = y), data = rug_base, sides = 'b', inherit.aes = F, col = rgb(.5,0,0, alpha = .1), length = unit(0.04, "npc")) +
    geom_xsidedensity(data = rug_base, mapping = aes(y = after_stat(density)), position = "stack", outline.type = 'upper',  col = 'black') + 
...
ale_figS_lst[[i]] <- ale_figS_lst[[i]] + 
  theme(legend.position = c(0.8, 0.8), legend.background = element_rect(fill = NA), legend.key = element_rect(fill = NA, color = NA)) + 
  guides(colour = guide_legend(override.aes = list(linewidth = 1, alpha = 1))) +
  labs(color = 'Tuning spaces')

plot_grid(plotlist = lapply(seq_along(ale_figS_lst), function(i){ale_figS_lst[[i]] + xlim(quantile(X[[i]],  probs=c(0.05, .95)))}), ncol = 2) %>% 
  ggsave(filename = paste0(reddir,'/figs/red_river_figS1.pdf'), width = 8, height = 8)
