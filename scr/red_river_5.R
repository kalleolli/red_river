...
load(paste0(reddir,'/dat/bm_grids.rda')) # loads benchmark grid grd
...
grd_sample <- dplyr::bind_rows(bm_grid_tuned)

message("grd_sample with length ", nrow(grd_sample))

all_imp_mcl <- mclapply(1:nrow(grd_sample),function(i){         # i loop: 1 to 3600
